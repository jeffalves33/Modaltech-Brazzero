...
export function AppLayout({ children }: AppLayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-[#f5f5f5]">
      ...
      {/* CONTAINER GERAL: EMPURRA TUDO 73PX PRA BAIXO */}
      <div className="flex pt-[73px]">
        {/* SIDEBAR */}
        <aside
          className={cn(
            "w-[232px] bg-[#1a1a1a] text-white flex flex-col fixed top-[73px] bottom-0 transition-transform duration-300 z-40",
          ...
          <nav className="flex-1 px-4 py-6 space-y-6 overflow-y-auto">
            {navItems.map((section) => (
              <div key={section.label}>
                <p className="text-[10px] font-semibold text-gray-400 mb-3 px-3">
                  {section.label}
                </p>
        ...
        {/* CONTEÚDO PRINCIPAL */}
        <main className="flex-1 w-full lg:ml-[232px] px-4 md:px-8 pb-8">
          {children}
        </main>
      </div>
    </div>
